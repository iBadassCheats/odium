using UnityEngine;

namespace Odium.Components;

public class ResourceWrapper
{
	public enum Icons
	{
		Heart,
		World,
		Logo
	}

	public static Sprite GetResource(Icons icon)
	{
		switch (icon)
		{
		case Icons.Heart:
		{
			string base3 = "";
			return SpriteUtil.Create(SpriteUtil.CreateTextureFromBase64(base3), new Rect(0f, 0f, 24f, 24f), new Vector2(0f, 0f));
		}
		case Icons.World:
		{
			string base2 = "";
			return SpriteUtil.Create(SpriteUtil.CreateTextureFromBase64(base2), new Rect(0f, 0f, 64f, 64f), new Vector2(0f, 0f));
		}
		case Icons.Logo:
		{
			string @base = "";
			return SpriteUtil.Create(SpriteUtil.CreateTextureFromBase64(@base), new Rect(0f, 0f, 64f, 64f), new Vector2(0f, 0f));
		}
		default:
			return null;
		}
	}
}
